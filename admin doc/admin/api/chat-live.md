...
- **Errors** — every error arrives in the canonical `ApiErrorResponse` envelope (`timestamp`, `status`, `error`, `message`, `path`, `errorCode`, `details?`, `traceId`); see [frontend-error-handling.md](../../../docs/errors/frontend-error-handling.md). Malformed UUID/date query params fail request binding with a 400 through the same envelope. Common per-surface codes are listed under each endpoint; 401 (no/expired token) and 403 (role missing) apply everywhere and are not repeated.
