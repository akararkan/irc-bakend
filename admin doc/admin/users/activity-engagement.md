...
> This is a distinct concern from [logs-audit.md](../platform/logs-audit.md): that section is
...
> [analytics-kpis.md](../platform/analytics-kpis.md) §12 and [logs-audit.md](../platform/logs-audit.md)
...
| **Active-contributor overlap** | users with ≥1 *create* activity (`POST_CREATED`/`RESEARCH_PUBLISHED`/`QNA_*_CREATED`) in window — feeds the WALC north-star | **[PLANNED]** — see [analytics-kpis.md](../platform/analytics-kpis.md) §4.1 |
| **SSE stream health** | live `irc:activity:{userId}` connection count, heartbeat success, emitter churn | **[PARTIAL]** — heartbeat exists; a metrics counter is the build (see [operations.md](../platform/operations.md) §6.2) |
...
event pipeline** today ([analytics-kpis.md](../platform/analytics-kpis.md) §6), yet this ledger
...
as a candidate collector in [analytics-kpis.md](../platform/analytics-kpis.md) §6.3.
...
surfacing to [safety-reports.md](../trust-safety/safety-reports.md):
...
| Mention-spam corroboration | `MENTION_LOOKUP` bursts | pairs with the mention-abuse view in [content-moderation.md](../trust-safety/content-moderation.md) |
...
| Admin reads/erasures of activity | Cassandra audit (`AuditLogService.record`) | every A1–A5 call writes an audit row → [logs-audit.md](../platform/logs-audit.md) |
...
| Activity write lag | RabbitMQ notification consumer backlog (activity is fed from it) | timeline going stale — see [operations.md](../platform/operations.md) §5 |
...
- **Default is no access.** A user's activity timeline is among the most sensitive
  ...
  viewed, what they watched. Per the privacy contract (top of doc + [analytics-kpis.md](../platform/analytics-kpis.md)
