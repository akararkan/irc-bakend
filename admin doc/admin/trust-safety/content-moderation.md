...
[media-storage.md](../content/media-storage.md); research/QnA lifecycle in
[research-qna.md](../content/research-qna.md); chat/channel/live content boundaries in
[chat-channels-live.md](../communication/chat-channels-live.md). Tag legend and ground rules:
[README.md](../README.md).
...
| Post takedown/restore via the real `PostStatus` machine | User suspend/ban → [users-roles.md](../users/directory-and-roles.md) |
| Comment moderation (hard-delete model) | Media scanner implementation & failed-asset queue → [media-storage.md](../content/media-storage.md) |
...
> **A second, separate queue now exists.** The automated text-moderation system
> (built 2026-08-08) adds `GET /api/v1/admin/moderation/review` —
> `AdminAutoModerationController` — for content the toxicity classifier could not
> decide on its own. It is deliberately **not** merged into this inbox: the two
> ask different questions. This one asks "was this report valid?" about content
> that is already live; that one asks "is this over the line?" about content that
...
| Resource audit trail | `audit_log_by_resource` — written on every request and now readable: `GET /api/v1/admin/audit/resources/{resourceType}/{resourceId}` (`AuditLogController.resourceHistory`, built 2026-08) **[EXISTS]**; also catalogued in [logs-audit.md](../platform/logs-audit.md) |
...
> [sound-library.md](../content/sound-library.md) (Section 13) — catalog & category
...
| Step | Status | Detail |
|------|--------|--------|
| Create | **[EXISTS]** (2026-08-08) `POST /api/v1/admin/sounds` (`AdminSoundController`) | **Admin-curated only — no end-user upload path.** `ADMIN`/`MODERATOR`, lands straight on `APPROVED`. The old `POST /api/v1/sounds` upload route (`CassandraSoundController`) still exists but is now `ADMIN`/`MODERATOR`-only and `@Deprecated`; a regular user's request is `403`. See [sound-library.md](../content/sound-library.md) and [api/sounds-media.md](../api/sounds-media.md) |
...
> The blocklist is now the **fast** lever of a two-lever system. It still takes
...
> surface, not just the four Cassandra creates.
...
([architecture.md](../foundation/architecture.md)). Every action writes an audit row via
...
| `audit_log_by_resource` | Cassandra | "What happened to this post?" — written on every request, read via `GET /api/v1/admin/audit/resources/{type}/{id}` | **[EXISTS]** (read built 2026-08 — [logs-audit.md](../platform/logs-audit.md)) |
| `media_assets` failure rows | PG | FAILED_MODERATION feeder detail | **[PARTIAL]** ([media-storage.md](../content/media-storage.md)) |
...
reads — see [analytics-kpis.md](../platform/analytics-kpis.md). Honest sourcing per metric:
...
- **ADMIN-gated, normalized.** The stray `CassandraSoundController` /
  ...
  The `Role` enum has since widened (see [users-roles.md](../users/directory-and-roles.md)) and
...
- **Content-privacy boundary**: this section renders posts/comments/stories/
  ...
  DM previews, or live-stream chat ([chat-channels-live.md](../communication/chat-channels-live.md)).
...
| 2 | **Sound reject/archive endpoints** + wire the dead `SoundApprovedEvent` (publish on approve, add `SoundRejectedEvent`) → uploader notifications | 1 | Completes the `SoundStatus` machine; new `NotificationType` needs an `EmailTemplate.actionVerb` case ([notifications-email.md](../communication/notifications-email.md)) |
...
| 6 | **Unified queue read** over `reports` (content targets) + FAILED_MODERATION assets | safety triage endpoints ([safety-reports.md](safety-reports.md)); [media-storage.md](../content/media-storage.md) scanner | Queue *writes* (dismiss/assign) live with the report state machine |
...
| 10 | **Analytics collectors**: daily content-velocity rollup, keyword-hit rollup, latency from decision log | 3, 8 | Feeds [analytics-kpis.md](../platform/analytics-kpis.md) |
