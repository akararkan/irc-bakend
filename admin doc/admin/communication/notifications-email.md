...
| Digest job runs | app log **[EXISTS]** → `job_runs` ledger **[PLANNED]** ([operations.md](../platform/operations.md)) | |
...
| Notification queue lag | `irc.queue.notifications` depth growing ([operations.md](../platform/operations.md)) | consumer stall |
...
- Preference data is per-user configuration: show **aggregates** on the
  ...
  view ([users-roles.md](../users/directory-and-roles.md)) and reads are audited.
