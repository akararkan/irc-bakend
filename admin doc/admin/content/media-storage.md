...
and the MediaMTX plane — see [chat-channels-live.md](../communication/chat-channels-live.md);
sound *moderation* — see [content-moderation.md](../trust-safety/content-moderation.md).
...
(`{app.streaming.recordings-dir}/<stream-id>/*.mp4` — [chat-channels-live.md](../communication/chat-channels-live.md));
...
| C4 | **Sounds** → real upload: intent with `type=AUDIO`, approval flow keeps gating visibility ([content-moderation.md](../trust-safety/content-moderation.md)) |
...
| Top consumers | Top-50 owners by `SUM(stored_bytes)` with asset counts; click-through to the user in [users-roles.md](../users/directory-and-roles.md) |
...
| Worker pool gauges | private `ThreadPoolExecutor` in `MediaProcessingService` | **[PLANNED]** — not exposed; needs a getter or Micrometer binding (note: `/actuator/prometheus` is not web-exposed today — see [operations.md](../platform/operations.md)) |
...
(see [logs-audit.md](../platform/logs-audit.md)).
...
| App-log tags `[MEDIA]`, `[MEDIA-VID]` | console (no file appender) | worker completions, transcode fallbacks ("transcode disabled/unavailable — passthrough"), rendition-delete warnings | **[EXISTS]** (not queryable — see [logs-audit.md](../platform/logs-audit.md)) |
...
| Media DLQ | messages appear in `irc.queue.dead-letter` with media type ids | future consumers poisoning (see [operations.md](../platform/operations.md)) |
...
- Every endpoint here lives under `/api/v1/admin/media/**` → **double-gated**
  ...
  platform convention ([architecture.md](../foundation/architecture.md)). No phantom
...
- **Content vs metadata:** the asset browser shows metadata by default; actually
  *viewing* a user's media is a content access — log it distinctly
  ...
  [chat-channels-live.md](../communication/chat-channels-live.md). Note today's boundary quirk:
...
- Admin delete must be refcount-safe (L3 fix first) and must write both the
  ...
  ([safety-reports.md](../trust-safety/safety-reports.md), `Resolution.CONTENT_REMOVED`).
