...
> Sounds began life as a subsection of [content-moderation.md](../trust-safety/content-moderation.md)
...
in [admin-api-blueprint.md](../foundation/api-blueprint.md) §3.3. Search-index mechanics
in [search-feed-trending.md](../platform/search-feed-trending.md); storage/CDN for the audio
...
|----------|--------------------|
| The whole sound-library catalog: browse, inspect, curate | Report state machine / strikes on the *uploader* → [safety-reports.md](../trust-safety/safety-reports.md) |
...
> reference: [api/sounds-media.md](../api/sounds-media.md).

### 2.3 API surface today (`CassandraSoundController` @ `/api/v1/sounds`)

| Method | Path | Auth | Notes |
|---|---|---|---|
| POST | `/api/v1/sounds` | `hasAnyRole('ADMIN','MODERATOR')` *(was `isAuthenticated()`)* | **Deprecated legacy alias.** Uploader **always** the JWT caller (body `uploaderId` ignored). `autoApprove=true` honored for the (now always privileged) caller; omitted/`false` still creates `PENDING_REVIEW` — the one place that status is still reachable. Answers with `Deprecation`/`Link` headers pointing at `POST /api/v1/admin/sounds`. |
...
1. **Phantom roles — fixed.** The stray `hasAnyRole('ADMIN','MODERATOR','SUPER_ADMIN')`
   ...
   [users-roles.md](../users/directory-and-roles.md).)
...
The moderation-workflow spec is in [content-moderation.md](../trust-safety/content-moderation.md)
...
| Manipulation guardrails | Flag a sound whose adoptions spike from few distinct authors (bot ring) | `posts_by_sound.authorId` distinct-count **[PLANNED]** — ties into [search-feed-trending.md](../platform/search-feed-trending.md) |
...
> exclude control**, and links to [search-feed-trending.md](../platform/search-feed-trending.md)
...
| Escalation | Link to issue a strike ([safety-reports.md](../trust-safety/safety-reports.md) — `POST /api/v1/admin/safety/users/{userId}/strikes`, built 2026-08) | **[EXISTS]** endpoint / **[PLANNED]** UI link |
...
| Decision log (approve/reject/archive/takedown) | PG | `moderation_decisions` via `ModerationRecorder` (shared with [content-moderation.md](../trust-safety/content-moderation.md)) | **[EXISTS]** (built 2026-08) |
...
**Since 2026-08-08, creation is admin-only and always pre-approved.** The
`[*] --> PENDING_REVIEW` entry below is now reachable only through the
deprecated legacy alias (`POST /api/v1/sounds` without `autoApprove: true`) —
not part of the intended flow, kept for backward compatibility. Every
normal creation — `POST /api/v1/admin/sounds` or the bulk `/import` — goes
straight to `APPROVED`.

```mermaid
stateDiagram-v2
    [*] --> APPROVED: admin create / import (EXISTS, canonical)
    [*] --> PENDING_REVIEW: legacy upload alias, no autoApprove (deprecated)
...
|---|---|---|---|
| → `APPROVED` | admin `POST /admin/sounds` or `/import` | row + `sounds_by_category` + ES, in one step | **[EXISTS]** (built 2026-08-08 — the canonical path) |
| → `PENDING_REVIEW` | *(legacy only)* deprecated upload alias without `autoApprove` | row in `sounds_by_id` only; ES indexed (non-APPROVED → never surfaces) | **[EXISTS]**, deprecated path |
...
| hard delete | GDPR erasure of uploader's own content | purge all stores + `posts_by_sound` detach | **[EXISTS]** (built 2026-08 — `DELETE /api/v1/admin/sounds/{id}`); see [logs-audit.md](../platform/logs-audit.md) GDPR |
...
([architecture.md](../foundation/architecture.md)). Full cross-doc table in
[admin-api-blueprint.md](../foundation/api-blueprint.md) §3.3.
...
Cross-reference the flagship [logs-audit.md](../platform/logs-audit.md); here's the
...
[analytics-kpis.md](../platform/analytics-kpis.md)). Honest sourcing — most trend metrics
...
- **ADMIN-only, honestly.** Sound moderation is ADMIN-gated. The bare `approve`
  ...
  ([users-roles.md](../users/directory-and-roles.md)), so any moderator-tier grant must reference
...
- **Public-plane content only.** Sounds attach to reels/posts/stories — public
  ...
  in this section** (privacy boundary — [chat-channels-live.md](../communication/chat-channels-live.md)).
...
Mirrors the platform-wide 3-phase order ([admin-api-blueprint.md](../foundation/api-blueprint.md)).
...
| **2 — moderation actions** | reject / archive / restore / re-categorize / edit · wire `SoundApprovedEvent` + add `SoundRejectedEvent`/`SoundArchivedEvent` → uploader notifications · bulk moderate · uploader reputation panel | 1, 1b; new `NotificationType`s each need an `EmailTemplate.actionVerb` case ([notifications-email.md](../communication/notifications-email.md)) | Completes the `SoundStatus` machine that's half-built in the enum |
