...
|---|---|
| Seven roles: `USER` / `RESEARCHER` / `SCHOLAR` / `MODERATOR` / `SUPPORT` / `ANALYST` / `ADMIN` (`user/enums/Role`) **[EXISTS]** (built 2026-08) | Role picker is a 7-value radio. `MODERATOR`/`SUPPORT`/`ANALYST` are live staff tiers with per-section grants ([architecture.md](architecture.md) §6). Only `SUPER_ADMIN` remains phantom — never render it. |
