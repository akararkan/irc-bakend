...
> **`@Query` completeness pass** with N+1 fixes (row V), a **search-read
> performance pass** across every ES-backed surface (row W), and **sound
> file-upload creation** — admins can now add a sound by uploading the audio
> file directly instead of only pasting a URL (row X).
...
| U | ✅ RESOLVED (2026-08-10) — **channels couldn't be left or deleted**: `POST /conversations/{id}/leave` rejected channels outright ("group-only action"), and `DELETE /conversations/{id}` on a channel silently fell through to the DM "hide for me" branch — the owner had no way to actually delete a channel, and the doc's claim that it worked was wrong. Fixed: `leave` on a channel now delegates to `unsubscribe`; `delete` branches owner→`ChannelService.deleteChannel` (soft-delete + search de-index) / subscriber→`unsubscribe`; new `DELETE /api/v1/channels/{id}` | ../chat/channels/inbox.md, ../chat/conversations.md, ../chat/groups.md | — |
| V | ✅ RESOLVED (2026-08-10) — **admin-repo `@Query` completeness pass**: zero derived `findBy`/`countBy` remain on any admin GET path (16 shared repos + 7 admin repos converted, ~28+ methods); every paged query carries an explicit `countQuery`. Caught and fixed 3 real N+1s along the way: `/admin/ops/jobs` (one point-query per registered job → one native `DISTINCT ON` scan), `AdminContentService.postsByAuthor` (Cassandra point read per row → `findAllById` IN-bulk), and three per-enum count loops (chat overview, analytics tiles, message-request funnel → single `GROUP BY` each) | architecture.md §6, this file's own §"Repo hygiene" row | — |
| W | ✅ RESOLVED (2026-08-10) — **search read paths were paying for work nobody used**: every ES query (global search, chat message search, sound search) counted total hits it never rendered (`track_total_hits` defaults true) and shipped full `_source` (bodies, keyword arrays, counters) over the wire when only ids/preview fields were read. Plus real N+1s: sound picker + admin sound queue hydrated one Cassandra point-read per ranked id (now `getByIds`/`useCountsFor` IN-bulk), people search didn't join-fetch the profile it renders (now `findActiveWithProfileByIdIn`), and the user-reindex job point-counted followers per user (now one grouped `countByFollowingIdIn` per page) | ../search/algorithms-and-complexity.md §1.8, ../search/global-search.md, ../search/sounds.md, ../search/users.md | — |
| X | ✅ RESOLVED (2026-08-10) — **no way to add a sound by file, only by URL**: `POST /api/v1/admin/sounds` required a pre-hosted `audioUrl`; an admin with a raw audio file had nowhere to upload it. New `POST /api/v1/admin/sounds/upload` (multipart `file` + optional `cover`) — bytes land in R2 under `sounds/`, row stores the media-proxy URL, duration auto-extracted for MP4-container audio, orphan-safe on a mid-upload failure | ../admin/api/sounds-media.md, ../admin/content/sound-library.md §2.4 | — |
