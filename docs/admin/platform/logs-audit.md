...
> Siblings: [architecture.md](../foundation/architecture.md) ·
> [users-roles.md](../users/directory-and-roles.md) · [safety-reports.md](../trust-safety/safety-reports.md) ·
> [operations.md](operations.md) · [analytics-kpis.md](analytics-kpis.md) ·
> [admin-api-blueprint.md](../foundation/api-blueprint.md)
...
|----------|---------------------------|
| Request-level audit log (audit module: Cassandra tables, interceptor, admin API, live SSE tail) | Report **triage workflow** — [safety-reports.md](../trust-safety/safety-reports.md) (this doc covers reports/strikes only *as records*) |
...
| Moderation records (`reports`, `user_strikes`) and data-lifecycle records (`export_jobs`, `account_deletion_requests`, `deleted_accounts`) as evidence stores | Media pipeline ops — [media-storage.md](../content/media-storage.md) (media errors appear here only as a failure *trail*) |
...
| **Moderation trail** [PARTIAL] | Reports intake timeline, per-target report stacks (`group_key`), strikes ledger (live since 2026-08), appeal states | `reports` + `user_strikes` (§3.7); actions live in [safety-reports.md](../trust-safety/safety-reports.md) |
...
| Dashboard use | The row drawer in the user/content inspection views ([users-roles.md](../users/directory-and-roles.md)) shows this stamp per record; Log Explorer pivots "same IP" queries can hit these columns per-table [PLANNED] |
...
### 3.7 `reports` + `user_strikes` — moderation records [EXISTS as records; console is [safety-reports.md](../trust-safety/safety-reports.md)]
...
[safety-reports.md](../trust-safety/safety-reports.md).
...
| Dashboard | Infra view failure queue: `WHERE status IN (terminal failures) ORDER BY updated_at DESC` with `error_message`, owner, type, size — query is trivial, endpoint proposed in [media-storage.md](../content/media-storage.md) (A-refs there); this doc only claims the *trail* |
...
a feature. Boundary rationale: [chat-channels-live.md](../communication/chat-channels-live.md).
...
(double-gate convention — [architecture.md](../foundation/architecture.md)). Danger:
...
[safety-reports.md](../trust-safety/safety-reports.md) · job/queue health & actuator exposure →
