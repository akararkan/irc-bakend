...
(infra health metrics — *not* product analytics), [admin-api-blueprint.md](../foundation/api-blueprint.md)
(endpoint registry), [users-roles.md](../users/directory-and-roles.md) / [safety-reports.md](../trust-safety/safety-reports.md)
...
| The Overview page + per-module drilldowns of the dashboard's Analytics section | Moderation queues/decision metrics → [safety-reports.md](../trust-safety/safety-reports.md), [content-moderation.md](../trust-safety/content-moderation.md) |
...
| `reports`, `user_strikes` | Safety intake volume ([safety-reports.md](../trust-safety/safety-reports.md)) |
...
| Per-channel stats | members, online, joins 7/30d, views, top posts | `ChannelStatsService` **[EXISTS]** — but owner/admin-gated; the dashboard needs the platform-admin override proposed in [chat-channels-live.md](../communication/chat-channels-live.md) | Drilldown page | **[PARTIAL]** |
...
| `report.submit` | reporter | entity | reason | `ReportService` (volume only; triage lives in [safety-reports.md](../trust-safety/safety-reports.md)) |
...
| 12 | **Open reports** | SUBMITTED + APPEALED count | PG `reports.state` → [safety-reports.md](../trust-safety/safety-reports.md) | **[PARTIAL]** |
...
[chat-channels-live.md](../communication/chat-channels-live.md)). Entity inspector panes reuse
...
double-gate + `@PreAuthorize` ([architecture.md](../foundation/architecture.md)); every call
...
[chat-channels-live.md](../communication/chat-channels-live.md) before the channel drilldown works
...
| `research_downloads` (PG) / `research_downloads_by_research` (Cassandra) | Download drilldown per research | **[EXISTS]** (repo reads; no admin endpoint yet → [research-qna.md](../content/research-qna.md)) |
...
| **No content in props, ever** | `chat.message_send` carries type/conversation-type only; the privacy boundary of [chat-channels-live.md](../communication/chat-channels-live.md) holds for analytics too. Never surface `conversations.last_message_preview` in any analytics projection (known metadata leak) |
...
| Deleted users | Rollups keep aggregate counts; `analytics_events` rows for purged accounts are anonymized-by-TTL (400d) — document in the deletion policy ([users-roles.md](../users/directory-and-roles.md)) |
...
stats override ([chat-channels-live.md](../communication/chat-channels-live.md)); report tiles
consume the triage model of [safety-reports.md](../trust-safety/safety-reports.md); rollup-job
monitoring registers in the jobs inventory of [operations.md](operations.md);
all endpoints enter the registry in [admin-api-blueprint.md](../foundation/api-blueprint.md).
