...
|----------|--------------------|
| Privacy boundary: what admins may ever see of chat | Report triage queue mechanics → [safety-reports.md](safety-reports.md) |
| Channel oversight: verification, stats, join requests, invite links, takedown | Chat message search index ops → [search-feed-trending.md](search-feed-trending.md) |
| Groups/DMs: aggregate stats + message-request quarantine stats | R2 media objects & media pipeline → [media-storage.md](media-storage.md) |
| Calls: volume/duration/missed analytics | MediaMTX process health / env vars → [operations.md](operations.md) |
...
| ES `irc-chat-messages` | Second content store; reachable via global search plumbing | Dashboard never queries it; reindex ops stay in [search-feed-trending.md](search-feed-trending.md) |
...
[safety-reports.md](safety-reports.md)) or a legal hold (§2.2), and even then shows
...
recipients** → feed into [safety-reports.md](safety-reports.md) triage as a signal source.

---

## 6. Admin actions

Convention (per [architecture.md](architecture.md)): all new routes under
...
Full catalog: [logs-audit.md](logs-audit.md). This section's log panel filters to:
...
**Honesty note (per [analytics-kpis.md](analytics-kpis.md)):** nothing in chat is
...
| Missed-call anomaly | MISSED share > 60% platform-wide / 1 h | `call_sessions` | Signaling/SSE health check → [operations.md](operations.md) |
...
| **3b** | Sampling collectors (concurrent-live, viewers-now time series), publish-auth failure counter, alert rules §9 | 1a/1b; a metrics store decision from [analytics-kpis.md](analytics-kpis.md) | Low |
...
Cross-references: existing admin inventory in [architecture.md](architecture.md) ·
endpoint master list in [admin-api-blueprint.md](admin-api-blueprint.md) · report triage in
[safety-reports.md](safety-reports.md) · log catalog in [logs-audit.md](logs-audit.md) ·
MediaMTX/infra runbooks in [operations.md](operations.md).
