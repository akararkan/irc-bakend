...
> **Relationship to the [API blueprint](api-blueprint.md):** the blueprint is
...
[operations.md](../platform/operations.md) §7 (the `permit-all` kill-switch, env registry).
...
- **This is the class the full add-users & full-control surface extended** — see
  [user-administration.md](../users/administration.md) and §6.1 below.
...
- Data comes from the Cassandra `audit_log_by_user` table (180-day TTL), written by
  the HTTP interceptor (§5). Detail: [logs-audit.md](../platform/logs-audit.md).
...
|----------|-----------|------|---------|
| `POST /api/v1/sounds/{id}/approve` | `post/cassandra/controller/CassandraSoundController` (base `/api/v1/sounds`) | `hasRole('ADMIN')` (normalized 2026-08) + `@Deprecated` | outside prefix; deprecated with `Deprecation`/`Link` successor headers → `POST /api/v1/admin/sounds/{id}/approve` **[EXISTS]** (built 2026-08; [sound-library.md](../content/sound-library.md) §6, [blueprint §3.3](api-blueprint.md)) |
| `POST /api/v1/sounds` | `post/cassandra/controller/CassandraSoundController` | `hasAnyRole('ADMIN','MODERATOR')` (was `isAuthenticated()`) + `@Deprecated` | **Closed 2026-08-08.** Was an open end-user upload path into a library every post/reel/story can pick from. Now staff-only and deprecated with a successor `Link` → `POST /api/v1/admin/sounds`; a regular user gets 403. It remains the only route that can still create a `PENDING_REVIEW` sound (when `autoApprove` is omitted) — the canonical admin create is always pre-approved ([sound-library.md](../content/sound-library.md) §2.3) |
...
The blueprint's **stray-endpoint regression** guardrail ([§7](api-blueprint.md))
...
|---------------------|-----------|-------------|-----|--------|-----------------------|
| `AdminUserController` *(extended the existing class)* | `/api/v1/admin/users` | 3.1 | [user-administration.md](../users/administration.md), [users-roles.md](../users/directory-and-roles.md) | **[EXISTS]** — extended (built 2026-08) | `UserProvisioningService.provision` (ex-`AuthServiceImpl.register`), `AccountLifecycleService`, `RefreshTokenRepository`, `TwoFactorService`, `StrikeService`, `StepUpService` |
| `AdminContentController` | `/api/v1/admin/content` | 3.2 | [content-moderation.md](../trust-safety/content-moderation.md) | **[EXISTS]** (built 2026-08) | `CassandraPostService`, `CassandraCommentService`, `CassandraStoryService`, `PostHydrator` |
| `AdminModerationController` | `/api/v1/admin/moderation` | 3.2 | [content-moderation.md](../trust-safety/content-moderation.md) | **[EXISTS]** (built 2026-08) | the reports/media/keyword inbox + capped bulk actions |
| `AdminAutoModerationController` | `/api/v1/admin/moderation/review` | — | [automated-moderation.md](../trust-safety/automated-moderation.md) | **[EXISTS]** (built 2026-08-08) | `ContentModerationService`, `ModerationMetricsService`, `ModerationTrainingService` — the **proactive** classifier queue, distinct from the reports inbox above |
| `AdminModerationSettingsController` | `/api/v1/admin/moderation/settings` | — | [automated-moderation.md](../trust-safety/automated-moderation.md) | **[EXISTS]** (built 2026-08-08) | `ModerationSettingsService`, `ModerationDecisionEngine` (the dry-run replays stored scores through the same pure engine) |
| `AdminModerationModelController` | `/api/v1/admin/moderation/model` | — | [automated-moderation.md](../trust-safety/automated-moderation.md) | **[EXISTS]** (built 2026-08-08) | `ModerationTrainingService`, `ModerationInferenceClient`, `ModerationTrainingClient` — dataset, registry, retrain/promote/rollback |
| `AdminSoundController` | `/api/v1/admin/sounds` | 3.3 | [sound-library.md](../content/sound-library.md) | **[EXISTS]** (built 2026-08; stray approve re-homed; `POST` create added 2026-08-08 when sounds became admin-curated only) | `CassandraSoundService` |
| `AdminResearchController` · `AdminQnaController` | `/api/v1/admin/research` · `/api/v1/admin/qna` | 3.4 | [research-qna.md](../content/research-qna.md) | **[EXISTS]** (built 2026-08) | `ResearchServiceImpl`/`QuestionServiceImpl` (already had an ADMIN ownership bypass, now explicit+audited) |
| `AdminTagController` *(extended `TagAdminController`)* | `/api/v1/admin/tags` | 3.4 | [research-qna.md](../content/research-qna.md) | **[EXISTS]** — extended (built 2026-08: hide/merge) | `ContentTagService`, trending suppression |
...
| `AdminSafetyController` | `/api/v1/admin/safety` | 3.6 | [safety-reports.md](../trust-safety/safety-reports.md) | **[EXISTS]** (built 2026-08) | `StrikeService` (now has callers), `ReportModerationService`, report/restriction repos |
| `AdminMediaController` · `AdminStorageController` | `/api/v1/admin/media` · storage | 3.7 | [media-storage.md](../content/media-storage.md) | **[EXISTS]** (built 2026-08) | `StorageUsageService`, media pipeline repos |
| `AdminNotificationController` | `/api/v1/admin/notifications` | 3.8 | [notifications-email.md](../communication/notifications-email.md) | **[EXISTS]** (built 2026-08) | notification services + the announcement composer |
| `SearchAdminController` *(exists)* · `AdminFeedController` | `/api/v1/admin/search` · `/api/v1/admin/feed` | 3.9 | [search-feed-trending.md](../platform/search-feed-trending.md) | **[EXISTS]** / **[EXISTS]** (built 2026-08) | the 7 reindexers; `FeedRankingService` knob reflection |
| `AuditLogController` *(exists)* | `/api/v1/admin/audit` | 3.10 | [logs-audit.md](../platform/logs-audit.md) | **[EXISTS]** | `AuditLogByUserRepository`, `AuditRealtimeService` |
| `AdminAnalyticsController` | `/api/v1/admin/analytics` | 3.11 | [analytics-kpis.md](../platform/analytics-kpis.md) | **[EXISTS]** (built 2026-08) | the `analytics_events` collector + daily rollups |
| `AdminOpsController` | `/api/v1/admin/ops` | 3.12 | [operations.md](../platform/operations.md) | **[EXISTS]** (built 2026-08) | health probes, job-run ledger, `MediaControlClient`, queue/DLQ browse |
| `AdminDiscoveryController` · `AdminSuggestionsController` | `/api/v1/admin/discovery` · `/api/v1/admin/suggestions` | 3.13 | [discovery-pymk-privacy.md](../users/discovery-privacy.md) | **[EXISTS]** (built 2026-08) | `FriendSuggestionService.recomputeFor`, `ContactMatchService`, `ConsentService` |
| `AdminKnowledgeController` | `/api/v1/admin/knowledge` | 3.14 | [knowledge-vocabulary.md](../content/knowledge-vocabulary.md) | **[EXISTS]** (built 2026-08) | `TopicRepository`/`MadhhabRepository` (their **first** `save`/retire callers) + cache eviction |
| `AdminActivityController` | `/api/v1/admin/users/{id}/activity` etc. | 3.15 | [activity-engagement.md](../users/activity-engagement.md) | **[EXISTS]** (built 2026-08) | `UserActivityService` (erase); break-glass reads — **default deny**, dual-control `BreakGlassCase` |
...
([user-administration.md](../users/administration.md) §6):
...
6. **No secret ever in a response DTO** — `password`, `two_factor_secret`, refresh-token
   ...
   a build-fail arch-test ([blueprint §7](api-blueprint.md)).
...
Consistent with [admin-api-blueprint.md §10](api-blueprint.md) and
