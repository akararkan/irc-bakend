...
Companion docs: [architecture.md](architecture.md) (access model),
[logs-audit.md](logs-audit.md) (log catalog),
[admin-api-blueprint.md](admin-api-blueprint.md) (all endpoints + phasing),
[media-storage.md](media-storage.md) (R2/media pipeline detail),
...
| All 16 `@Scheduled` methods + the continuous listeners; job controls | Media pipeline queues' *business* view → [media-storage.md](media-storage.md) |
...
| Redis key-prefix registry, memory, rate-limiter observability | User-facing storage quotas → [media-storage.md](media-storage.md) |
...
| R2 | Bucket versioning + lifecycle (abort incomplete multipart 7d, transition cold originals); documented in [media-storage.md](media-storage.md) |
...
- All new surfaces under `/api/v1/admin/ops/**` → inherit the
  ...
  gate ([architecture.md](architecture.md)). Do **not** repeat the
