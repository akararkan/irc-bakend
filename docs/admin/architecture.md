...
| Dashboard app shape (SPA, SSE live tiles, auth flow) | Log catalog details — [logs-audit.md](logs-audit.md) |
| API conventions binding on ALL new `/api/v1/admin/**` endpoints | KPI definitions — [analytics-kpis.md](analytics-kpis.md) |
| Inventory of every existing admin surface | Full endpoint list — [admin-api-blueprint.md](admin-api-blueprint.md) |
| RBAC evolution (sub-admin roles), impersonation policy | Ops/runbooks — [operations.md](operations.md) |
...
| Visibility | Target-user notification is a product decision; the audit trail is not optional. Impersonation events surface in [logs-audit.md](logs-audit.md) as a first-class filter and count as a KPI in §11 of that doc. |
...
writer, retention — is [logs-audit.md](logs-audit.md).
...
| 1 | [users-roles.md](users-roles.md) | User directory/inspection, roles & badges, account controls, sessions/2FA, deletion pipeline, growth |
| 2 | [content-moderation.md](content-moderation.md) | Post/comment/story/reel queues, sound approval queue, platform keyword blocklist, bulk actions |
| 3 | [research-qna.md](research-qna.md) | Research pipeline/IRC-id/downloads, QnA oversight, tags & trending admin |
| 4 | [chat-channels-live.md](chat-channels-live.md) | Chat privacy boundary, channel verification/stats, invites, live-stream force-stop/keys/recordings, gifts |
| 5 | [safety-reports.md](safety-reports.md) | Report triage queue, strikes, appeals, consent viewer, moderation SLAs |
| 6 | [media-storage.md](media-storage.md) | Media pipeline board, failed-media queues, dedup/tiers, storage usage, R2 lifecycle |
| 7 | [notifications-email.md](notifications-email.md) | Notification volume, email deliverability, digest job, announcement composer |
| 8 | [search-feed-trending.md](search-feed-trending.md) | ES index health + the 7 reindexes, trending controls, feed-ranking observability, suggestions |
| 9 | [logs-audit.md](logs-audit.md) | Complete log-store catalog, unified Log Explorer, retention, alert rules |
| 10 | [analytics-kpis.md](analytics-kpis.md) | KPI tree, per-module metrics with honest sourcing, event-collection proposal |
| 11 | [operations.md](operations.md) | Dependency health, 16 scheduled jobs, RabbitMQ/DLQ, SSE/Redis ops, env registry, runbooks |
| 12 | [admin-api-blueprint.md](admin-api-blueprint.md) | Every admin endpoint (existing + proposed), danger levels, phased build order |
...
- **Admins never see message content**: chat/DM bodies (Cassandra `message_by_conversation`, ES `irc-chat-messages`, recordings, R2 media) are out of bounds; even `conversations.last_message_preview` must be excluded from admin projections. Canonical statement in [chat-channels-live.md](chat-channels-live.md).
...
Full sequencing with per-endpoint ordering: [admin-api-blueprint.md](admin-api-blueprint.md).
